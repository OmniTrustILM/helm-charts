...
- The Java package rename from `com.czertainly` to `com.otilm` is only partially rolled out, so the logging env var in each deployment template has to name the namespace actually used by that chart's pinned `appVersion` — otherwise `logging.level` silently has no effect. Setting both spellings is also valid and is the safe option while a component is mid-migration. Verify with `gh api repos/OmniTrustILM/<repo>/contents/src/main/java/com/otilm?ref=<appVersion>` before bumping an `appVersion`, and update the template in the same commit. Current state of the templates:
