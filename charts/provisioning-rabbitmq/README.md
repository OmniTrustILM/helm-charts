...
| bootstrap.proxy.exchange                     | `"ilm-proxy"`                     | RabbitMQ exchange name for proxy communication. When deployed as a subchart of `ilm` with proxy support enabled, it must equal `global.provisioning.queue.exchange`; the umbrella chart fails rendering if the two differ |
